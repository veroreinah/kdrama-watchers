<template>
  <div class="pt-2">
    ...
    <v-dialog v-model="dialog" max-width="500">
      <v-card>
        <v-toolbar v-if="selectedDate" color="secondary" dark>{{
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn depressed tile color="secondary" @click="dialog = false">
            Aceptar
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  computed: {
    ...
    filteredKdramas() {
      if (this.selectedDate) {
        ...
        result.sort(
          (kdramaA, kdramaB) =>
            this.getListProp(kdramaA.data.list, "value") -
            this.getListProp(kdramaB.data.list, "value")
        );
  ...
  updated() {
    if (this.$refs.calendar) {
      ...
      this.disablePrev =
        start.month - 1 <= this.minDate.getMonth() &&
        start.year <= this.minDate.getFullYear();
      this.disableNext =
        end.month - 1 >= this.maxDate.getMonth() &&
        end.year >= this.maxDate.getFullYear();
    }
  },
};
</script>
